...
# Copyright (c) 2016 The Fleet Credits Core developers
...
# Dependencies
ossPatchUrl="https://depends.fleetcreditscore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.fleetcreditscore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.fleetcreditscore.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.fleetcreditscore.org/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
...
SIGNER=
VERSION=
url=https://github.com/fleetcredits/fleetcredits
proc=2
mem=2000
...
Standalone script to perform the gitian build of Fleet Credits Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/fleetcredits/fleetcredits
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/fleetcredits-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/fleetcredits/gitian.sigs.git
    git clone https://github.com/fleetcredits/fleetcredits-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/fleetcredits-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean fleetcredits git directory because of old caching
    if [ -d inputs/fleetcredits/ ]; then
        echo "Cleaning Fleet Credits directory..."
        rm -rf inputs/fleetcredits/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit fleetcredits="$COMMIT" --url fleetcredits="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
