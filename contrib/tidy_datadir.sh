#!/bin/bash
# Copyright (c) 2013 The Fleet Credits Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Fleet Credits database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Fleet Credits datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Fleet Credits datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Fleet Credits 0.7 datadir."
    ;;
  3)
    echo "Detected Fleet Credits pre-0.8 datadir."
    ;;
  4)
    echo "Detected Fleet Credits 0.8 datadir."
    ;;
esac
