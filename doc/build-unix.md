# Unix Build of Fleet Credits Core
...
You must install required dependencies to build a basic Fleet Credits daemon, optional dependencies may vary according to your requirements.
...
Fleet Credits Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :
```bash
# Qt (required for fleetcredits-qt GUI)
...
According to installed dependencies, the following steps will compile `fleetcreditsd`, `fleetcredits-cli` and `fleetcredits-qt`.
...
#### Enable Qt GUI
Create `fleetcredits-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Fleet Credits Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Fleet Credits Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Fleet Credits may be compiled in
...
#### Security
To help make your Fleet Credits installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Fleet Credits should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Fleet Credits Core.
