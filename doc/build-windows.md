...
Below are some notes on how to build Fleet Credits Core for Windows.
...
To build Fleet Credits from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Fleet Credits GitHub repository](https://github.com/fleetcredits/fleetcredits/)).
...
Note that for WSL v1 the Fleet Credits Core source path MUST be somewhere in the default mount file system, for
example /usr/src/fleetcredits, AND not under, for example, /mnt/d/fleetcredits.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Fleet Credits's dependencies):
```bash
...
way. This will install to `c:\workspace\fleetcredits`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/fleetcredits
```
