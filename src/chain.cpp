...
CBlockHeader CBlockIndex::GetBlockHeader(const Consensus::Params& consensusParams, bool fCheckPOW) const
{
    ...
    if (block.IsAuxpow())
    {
        // For non-server builds (like fleetcredits-tx), we can't read auxpow from disk
        // Return incomplete header - caller should not rely on auxpow data in this case
        // The real ReadBlockHeaderFromDisk is only available when linking server library
        if (pprev)
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        // Note: auxpow data is not available - this is incomplete
        return block;
    }
