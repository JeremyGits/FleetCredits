...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s fleetcredits-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  fleetcredits-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded fleetcredits transaction") + "\n" +
              "  fleetcredits-tx [options] -create [commands]   " + _("Create hex-encoded fleetcredits transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CFleetCreditsAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CFleetCreditsAddress addr(scriptPubKey);
    ...
    if (bScriptHash) {
        ...
        CFleetCreditsAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded fleetcredits transaction
