 ...
 * between fleetcreditsd, and fleetcredits-qt and non-server tools.
*/
...
public:
    ...
    bool GetCoins(const uint256 &txid, CCoins &coins) const {
        try {
            ...
            uiInterface.SafeThreadSafeMessageBox(_("Error reading from database, shutting down."), "", CClientUIInterface::MSG_ERROR);
...
void Shutdown()
{
    ...
    RenameThread("fleetcredits-shutoff");
    ...
    StopHTTPServer();
#ifdef ENABLE_WALLET
    // Fleet Credits 1.14 TODO: ShutdownRPCMining();
    if (pwalletMain)
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), FLEETCREDITS_CONF_FILENAME));
    if (mode == HMM_FLEETCREDITSD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), FLEETCREDITS_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/fleetcredits/fleetcredits>";
    const std::string URL_WEBSITE = "<https://fleetcredits.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("fleetcredits-loadblk");
...
/** Sanity checks
 *  Ensure that Fleet Credits is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Fleet Credits version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Fleet Credits process is using the data directory.
    ...
    try {
        ...
        if (!lock.try_lock()) {
            // Use SafeThreadSafeMessageBox directly to avoid signal issues
            uiInterface.SafeThreadSafeMessageBox(
                strprintf(_("Cannot obtain a lock on data directory %s. %s is probably already running."), strDataDir, _(PACKAGE_NAME)),
        ...
        // Use SafeThreadSafeMessageBox directly to avoid signal issues
        uiInterface.SafeThreadSafeMessageBox(
            strprintf(_("Cannot obtain a lock on data directory %s. %s is probably already running.") + " %s.", strDataDir, _(PACKAGE_NAME), e.what()),
            "", CClientUIInterface::MSG_ERROR);
        return false;
    }
    return true;
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", FLEETCREDITS_CONF_FILENAME)).string());
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // For regtest, allow reindexing if genesis doesn't match
                if (!mapBlockIndex.empty() && mapBlockIndex.count(chainparams.GetConsensus(0).hashGenesisBlock) == 0)
                {
                    if (chainparams.NetworkIDString() == "regtest") {
                        LogPrintf("Regtest: Genesis block mismatch detected. Clearing old data and reindexing...\n");
                        // Clear old chain data for regtest
                        UnloadBlockIndex();
                        delete pcoinsTip;
                        delete pcoinsdbview;
                        delete pcoinscatcher;
    ...
    if (fPruneMode) {
        ...
        if (!fReindex) {
            uiInterface.SafeInitMessage(_("Pruning blockstore..."));
            PruneAndFlush();
        }
    }
    ...
    // Fleet Credits: Do we need to do any RPC mining init here?

    SetRPCWarmupFinished();
    uiInterface.SafeInitMessage(_("Done loading"));

    // Notify GUI of current chain tip after initialization (like original client)
    // This ensures the debug window and other UI elements show correct block count
    {
        LOCK(cs_main);
        CBlockIndex* tip = chainActive.Tip();
        if (tip) {
            uiInterface.NotifyBlockTip(IsInitialBlockDownload(), tip);
        }
    }
    ...
    // Critical: Check if shutdown was requested during initialization
    // If so, log it but don't prevent return - let WaitForShutdown handle it
    if (ShutdownRequested()) {
        LogPrintf("WARNING: Shutdown was requested during initialization, but continuing to WaitForShutdown\n");
        // Don't return false here - let WaitForShutdown handle the shutdown gracefully
        // This allows the daemon to properly clean up instead of exiting immediately
    }

    return true;  // Always return true if we got here - initialization succeeded
}
