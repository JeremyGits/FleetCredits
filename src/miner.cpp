...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    // Extract contributions from block being created
    std::vector<CContributionTransaction> contributions = ExtractContributionsFromBlock(*pblock);
    
    // Get base reward
    CAmount baseReward = GetFleetCreditsBlockSubsidy(nHeight, consensus, pindexPrev->GetBlockHash());
    
    // Calculate total reward with contribution bonuses
    CAmount totalReward = baseReward;
    if (!contributions.empty()) {
        totalReward = GetFleetCreditsBlockSubsidyWithContributions(
            nHeight, 
            consensus, 
    ...
    // Start with miner's base reward + fees + portion of bonus
    // Miner gets: base reward + fees + 50% of bonus pool
    // Contributors get: remaining 50% of bonus pool distributed proportionally
    CAmount minerReward = nFees + baseReward;
    CAmount bonusAmount = totalReward - baseReward;
    
    if (bonusAmount > 0 && !contributions.empty()) {
        // Miner gets 50% of bonus, contributors split the other 50%
        CAmount minerBonusShare = bonusAmount / 2;
        minerReward += minerBonusShare;
    ...
    // Miner gets base reward + fees + miner's share of bonus
    // Add miner output last (standard practice: miner output is last)
    CTxOut minerOutput(minerReward, scriptPubKeyIn);
    coinbaseTx.vout.push_back(minerOutput);
    ...
    // Create MWEB extension block if there are MWEB transactions, peg-ins, or peg-outs
    CMWEBExtensionBlock mweb_block;
    bool has_mweb_data = false;
    
    // Collect pending MWEB transactions from mempool
    extern CMWEBMempool mwebMempool;
    std::vector<CMWEBTransaction> pending_mweb_txs = mwebMempool.GetMWEBTransactions();
    std::vector<CPegInTransaction> pending_peg_ins = mwebMempool.GetPegIns();
    std::vector<CPegOutTransaction> pending_peg_outs = mwebMempool.GetPegOuts();
    
    // Add pending MWEB transactions
    if (!pending_mweb_txs.empty()) {
        mweb_block.mweb_txs.insert(mweb_block.mweb_txs.end(), 
                                   pending_mweb_txs.begin(), 
                                   pending_mweb_txs.end());
        has_mweb_data = true;
    }
    
    // Add pending peg-ins from mempool (these already have recipient_address set)
    if (!pending_peg_ins.empty()) {
        mweb_block.peg_ins.insert(mweb_block.peg_ins.end(), 
                                  pending_peg_ins.begin(), 
                                  pending_peg_ins.end());
        has_mweb_data = true;
    }
    
    // Add pending peg-outs
    if (!pending_peg_outs.empty()) {
        mweb_block.peg_outs.insert(mweb_block.peg_outs.end(), 
                                    pending_peg_outs.begin(), 
                                    pending_peg_outs.end());
        has_mweb_data = true;
    }
    ...
    std::vector<CContributionTransaction> mweb_contribs;
    for (const auto& contrib : contributions) {
        if (contrib.RequiresMWEB()) {
            CMWEBTransaction mweb_tx;
            if (RouteToMWEB(contrib, mweb_tx)) {
                mweb_block.mweb_txs.push_back(mweb_tx);
                has_mweb_data = true;
            }
        }
    }
    ...
    if (has_mweb_data && (!mweb_block.mweb_txs.empty() || !mweb_block.peg_ins.empty() || !mweb_block.peg_outs.empty())) {
        ...
        if (pindexPrev) {
            ...
            CBlock prev_block;
            if (ReadBlockFromDisk(prev_block, pindexPrev, Params().GetConsensus(0))) {
                if (prev_block.mweb_extension) {
                    mweb_block.prev_mweb_hash = prev_block.mweb_extension->GetHash();
                } else {
