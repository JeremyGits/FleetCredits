    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/fleetcredits/fleetcredits/issues/5716
...
{
    GUIUtil::setClipboard(FleetCreditsUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = FleetCreditsUnits::FC;
    ...
    l2->setText(FleetCreditsUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(FleetCreditsUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(FleetCreditsUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(FleetCreditsUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CFleetCreditsAddress(outputAddress).ToString());

                // if listMode or change => show fleetcredits address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, FleetCreditsUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, FleetCreditsUnits::format(nDisplayUnit, nSum));
