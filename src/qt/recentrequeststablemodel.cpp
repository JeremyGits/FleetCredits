    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return FleetCreditsUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, FleetCreditsUnits::separatorNever);
            else
                return FleetCreditsUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+FleetCreditsUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
