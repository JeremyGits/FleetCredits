...
{
    if (!headers) {
        // Handle uninitialized chain (-1) by showing "Loading..." like the original client
        if (count == -1) {
            ui->numberOfBlocks->setText(tr("Loading..."));
        } else {
...
void RPCConsole::on_addPeerClicked() 
{
    ...
    connect(win, &AddPeerDialog::peerAdded, [this]() {
        if (clientModel && clientModel->getPeerTableModel()) {
            // Multiple refresh attempts to catch peer when it connects
            // Connections can take time to establish
...
{    
    // Get both NodeId and Address for more reliable disconnection
    QList<QModelIndex> nodeIds = GUIUtil::getEntryData(ui->peerWidget, PeerTableModel::NetNodeId);
    QList<QModelIndex> addresses = GUIUtil::getEntryData(ui->peerWidget, PeerTableModel::Address);

    if(nodeIds.size() != 0 && addresses.size() != 0)
    {
        QString address = addresses[0].data().toString();
        NodeId nodeId = nodeIds[0].data().toLongLong();
        ...
        if(QMessageBox::Yes == QMessageBox::question(this, tr("Remove Peer"), tr("Are you sure you want to remove the peer: ") + address + "?", QMessageBox::Yes | QMessageBox::No))
        {
            ...
            if(!g_connman) {
                QMessageBox::warning(this, tr("Remove Peer"), tr("Error: Peer-to-peer functionality missing or disabled"), QMessageBox::Ok, QMessageBox::Ok);
                return;
            }
            
            // First, try to disconnect by NodeId (more reliable)
            if (g_connman->DisconnectNode(nodeId)) {
                disconnected = true;
            }
            
            // Also try to disconnect by address string (in case NodeId didn't work)
            if (!disconnected) {
                if (g_connman->DisconnectNode(address.toStdString())) {
                    disconnected = true;
                }
            }
            ...
            if (disconnected && removedFromList) {
                result = tr("Removed from list and disconnected: ") + address;
            } else if (removedFromList) {
                result = tr("Removed from persistent list: ") + address;
            } else if (disconnected) {
                result = tr("Disconnected (was not in persistent list): ") + address;
            } else {
                result = tr("Could not disconnect or remove peer. It may have already disconnected.") + "\n" + address;
            }
            
            QMessageBox::information(this, tr("Remove Peer"), result, QMessageBox::Ok, QMessageBox::Ok);
            
            // Force immediate refresh, then refresh again after a delay to catch reconnections
            if (clientModel && clientModel->getPeerTableModel()) {
                clientModel->getPeerTableModel()->refresh();
                // Refresh again after 1 second to see if peer stayed disconnected
...
void RPCConsole::disconnectSelectedNode()
{
    ...
    if (anyDisconnected) {
        ...
        // Force immediate refresh of peer list
        if (clientModel && clientModel->getPeerTableModel()) {
            clientModel->getPeerTableModel()->refresh();
        }
    }
