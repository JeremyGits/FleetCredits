    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  fleetcredits-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_FLEETCREDITS_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FleetCreditsGUI::DEFAULT_UIPLATFORM));
        }
...
void PaperWalletDialog::on_getNewAddress_clicked()
{
    ...
    CFleetCreditsAddress pubkeyhash;
    pubkeyhash.Set(pubkey.GetID());

    // Create String versions of each
    std::string myPrivKey = CFleetCreditsSecret(privKey).ToString();
...
void PaperWalletDialog::on_printButton_clicked()
{
    ...
    while (true) {
        ...
        // Ask for an amount to send to each paper wallet. It might be better to try to use the FleetCreditsAmountField, but this works fine.
        double amountInput = QInputDialog::getDouble(this, tr("Load Paper Wallets"), tr("The paper wallet printing process has begun.<br/>Please wait for the wallets to print completely and verify that everything printed correctly.<br/>Check for misalignments, ink bleeding, smears, or anything else that could make the private keys unreadable.<br/>Now, enter the number of FC you wish to send to each wallet:"), 0, 0, 2147483647, 8, &ok);
    ...
    if (txFee > 0) {
        ...
        questionString.append(FleetCreditsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH (FleetCreditsUnits::Unit u, FleetCreditsUnits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(FleetCreditsUnits::formatWithUnit(u, totalAmount));
    }

    questionString.append(tr("Total Amount %1 (= %2)")
                              .arg(FleetCreditsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
...
QWidget *ShutdownWindow::showShutdownWindow(FleetCreditsGUI *window)
{
    if (!window)
        return nullptr;
