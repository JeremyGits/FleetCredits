...
// Copyright (c) 2021-2025 The Fleet Credits Core developers
...
void AuxMiningCheck()
{
    ...
    if (IsInitialBlockDownload() && !Params().MineBlocksOnDemand())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Fleet Credits is downloading blocks...");
...
static UniValue AuxMiningCreateBlock(const CScript& scriptPubKey)
{
     ...
     * a single fleetcreditsd instance, for example when a pool runs multiple sub-
...
UniValue createauxblock(const JSONRPCRequest& request)
{
    ...
    CFleetCreditsAddress coinbaseAddress(request.params[0].get_str());
