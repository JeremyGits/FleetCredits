...
// Copyright (c) 2018-2024 The Fleet Credits Core developers
...
CTxMemPool::ReadFeeEstimates(CAutoFile& filein)
{
    try {
        ...
        if (nVersionRequired > CLIENT_VERSION) {
            LogPrintf("CTxMemPool::ReadFeeEstimates(): up-version (%d) fee estimate file, ignoring (non-fatal)\n", nVersionRequired);
            return false;  // Non-fatal: just ignore the old file
        }
        ...
        if (nVersionThatWrote < FEEFILE_MIN_COMPAT_VERSION_WRITER) {
            LogPrintf("CTxMemPool::ReadFeeEstimates(): cannot re-use fee estimate files from version %d, ignoring file (non-fatal)\n", nVersionThatWrote);
            return false;  // Non-fatal: just ignore incompatible file
        }
