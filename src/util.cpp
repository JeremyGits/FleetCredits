...
const char * const FLEETCREDITS_CONF_FILENAME = "fleetcredits.conf";
const char * const FLEETCREDITS_PID_FILENAME = "fleetcreditsd.pid";
...
#else
    const char* pszModule = "fleetcredits";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Fleet Credits
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Fleet Credits
    // Mac: ~/Library/Application Support/Fleet Credits
    // Unix: ~/.fleetcredits
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fleet Credits";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Fleet Credits";
#else
    // Unix
    return pathRet / ".fleetcredits";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No fleetcredits.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override fleetcredits.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", FLEETCREDITS_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Fleet Credits Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Fleet Credits Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Fleet Credits Core developers";
    }
