...
#if defined(NDEBUG)
# error "Fleet Credits cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Fleet Credits Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    // if we already know it's valid (e.g. it was already in the block chain)
    ...
    // Check for contribution transactions in OP_RETURN outputs
    // Contribution transactions are valid UTXO transactions with special OP_RETURN data
    // They don't need special validation here - they're validated when extracted from blocks
    // This allows contribution transactions to be accepted into mempool like normal transactions
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        // Fleet Credits: Uses GetFleetCreditsMinRelayFee which implements zero-fee for micro-transactions
        if (fLimitFree && nModifiedFees < GetFleetCreditsMinRelayFee(tx, nSize, !fLimitFree))
        {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s FC additional fees, %d delta bytes\n",
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    // Check the header - always validate PoW for all blocks
    // Genesis block PoW is validated during AcceptBlock and when written to disk
    // Skip PoW check for genesis block when reading from disk to avoid validation issues
    // The genesis block hash is already validated against chainparams
    if (fCheckPOW) {
        // Skip PoW validation for genesis block - it's already validated during InitBlockIndex
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Fleet Credits: Switch maturity at depth 145,000
...
bool AbortNode(const std::string& strMessage, const std::string& userMessage="")
{
    ...
    LogPrintf("*** %s\n", strMessage);
    uiInterface.SafeThreadSafeMessageBox(
...
void ThreadScriptCheck() {
    RenameThread("fleetcredits-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Fleet Credits: BIP30 has been active since inception
    ...
    // Fleet Credits: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetFleetCreditsBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
    
    // Extract contributions from block and calculate bonus rewards
    ...
    if (!contributions.empty()) {
        ...
        blockReward = nFees + GetFleetCreditsBlockSubsidyWithContributions(
            pindex->nHeight, 
    ...
    // Validate MWEB extension block if present
    if (block.mweb_extension) {
        if (!block.mweb_extension->VerifyAll()) {
            return state.DoS(100, false, REJECT_INVALID, "bad-mweb-extension");
        }
        
        // Verify peg-in transactions match main chain transactions
        for (const auto& peg_in : block.mweb_extension->peg_ins) {
            // Verify the main chain transaction exists in this block or previous blocks
            bool found_peg_tx = false;
            
            // Check current block first
            for (const auto& tx : block.vtx) {
                if (tx->GetHash() == peg_in.main_chain_tx_id) {
                    // Verify transaction has correct peg-in marker and amount
                    for (const auto& txout : tx->vout) {
                        ...
                        if (script.size() > 2 && script[0] == OP_RETURN && 
                            ...
                            try {
                                ...
                                ss >> peg_tx_id;
                                ss >> peg_amount;
                                if (peg_tx_id == peg_in.peg_tx_id && static_cast<uint64_t>(peg_amount) == peg_in.amount) {
                                    found_peg_tx = true;
                                    break;
                                }
                            } catch (...) {
                    ...
                    if (found_peg_tx) break;
                }
            }
            
            // If not in current block, verify it exists in previous blocks or mempool
            if (!found_peg_tx) {
                // Check if transaction is in mempool (pending)
                extern CTxMemPool mempool;
                {
                    LOCK(mempool.cs);
                    if (mempool.exists(peg_in.main_chain_tx_id)) {
                        // Transaction is in mempool, will be included in this or next block
                        // This is valid - peg-in can reference pending transaction
                        found_peg_tx = true;
                    }
                }
                
                // Also check previous blocks
                if (!found_peg_tx) {
                    CTransactionRef txOut;
                    uint256 hashBlock;
                    if (GetTransaction(peg_in.main_chain_tx_id, txOut, consensus, hashBlock, true)) {
                        // Transaction exists, verify it's confirmed
                        BlockMap::iterator it = mapBlockIndex.find(hashBlock);
                        if (it != mapBlockIndex.end()) {
                            CBlockIndex* pindexBlock = it->second;
                            if (pindexBlock && chainActive.Contains(pindexBlock)) {
                                found_peg_tx = true;
                            }
                        }
                    }
            ...
            if (!found_peg_tx) {
                return state.DoS(100, false, REJECT_INVALID, "bad-mweb-peg-in");
            }
        }
        
        // Verify peg-out transactions
        for (const auto& peg_out : block.mweb_extension->peg_outs) {
            ...
            if (!MWEB::VerifyCommitment(peg_out.mweb_input_commitment)) {
                return state.DoS(100, false, REJECT_INVALID, "bad-mweb-peg-out-commitment");
            }
            
            // Verify main chain address is valid
            if (!peg_out.main_chain_address.IsValid()) {
                return state.DoS(100, false, REJECT_INVALID, "bad-mweb-peg-out-address");
            }
            
            // Verify amount is positive
            if (peg_out.amount <= 0) {
                return state.DoS(100, false, REJECT_INVALID, "bad-mweb-peg-out-amount");
            }
            
            // View key proof verification would require decryption with the view private key
            // This is done externally for auditing purposes
            // Basic structure verification is sufficient for consensus
        }
        
        // Extract contributions from MWEB extension block
        // MWEB contributions are for privacy-sensitive types (e.g., ETHICAL_REVIEW)
        std::vector<CContributionTransaction> mweb_contribs = ExtractContributionsFromMWEB(*block.mweb_extension);
        if (!mweb_contribs.empty()) {
            // Add MWEB contributions to the total contributions list
            contributions.insert(contributions.end(), mweb_contribs.begin(), mweb_contribs.end());
        }
        
        // Clear MWEB mempool after block is connected (transactions are now in block)
        if (!fJustCheck) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Fleet Credits: Disallow AuxPow blocks before it is activated.
    ...
    // Fleet Credits: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Fleet Credits: We probably want to disable this
    int nLockTimeFlags = 0;
...
static bool __attribute__((unused)) IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned nRequired, const Consensus::Params& consensusParams)
{
...
CVerifyDB::CVerifyDB()
{
    uiInterface.SafeShowProgress(_("Verifying blocks..."), 0);
}

CVerifyDB::~CVerifyDB()
{
    uiInterface.SafeShowProgress("", 100);
}
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        uiInterface.SafeShowProgress(_("Verifying blocks..."), percentageDone);
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            boost::this_thread::interruption_point();
            uiInterface.SafeShowProgress(_("Verifying blocks..."), std::max(1, std::min(99, 100 - (int)(((double)(chainActive.Height() - pindex->nHeight)) / (double)nCheckDepth * 50))));
