...
/**
 * Fleet Credits: Effective dust limit for the wallet
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CFleetCreditsAddress(CScriptID(redeemScript)).ToString();
...
bool CWalletTx::IsEquivalentTo(const CWalletTx& _tx) const
{
        ...
        // Suppress false positive warnings about uninitialized CScript
        // CScript() constructor properly initializes the object
...
// Fleet Credits: MIN_CHANGE as a function of discardThreshold and minTxFee(1000)
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /*
                     * Fleet Credits: check all outputs against the discard threshold
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-fleetcredits-address
                ...
                for (auto& vin : txNew.vin) {
                    // Suppress false positive warning about uninitialized CScript
                    // CScript() constructor properly initializes the object
                    #pragma GCC diagnostic push
                    #pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
                    vin.scriptSig.clear();  // Clear first
                    vin.scriptSig = CScript();  // Then assign empty script (ensures proper initialization)
                    #pragma GCC diagnostic pop
                ...
                if (nPriority == MINIMUM) {
                    ...
                    nFeeNeeded = GetFleetCreditsPriorityFee(txNew, nBytes, nPriority);
                }
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    /* Fleet Credits: this has been changed from a static MIN_FINAL_CHANGE that
...
{
    // Fleet Credits: Add an increased fee for each output that is lower than the discard threshold
    return std::max(minTxFee.GetFee(nTxBytes) + GetFleetCreditsDustFee(tx.vout, discardThreshold), ::minRelayTxFeeRate.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    if (nFeeNeeded == 0) {
        ...
        // Fleet Credits: Drop the smart fee estimate, use GetRequiredFee
        nFeeNeeded = GetRequiredFee(tx, nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // Fleet Credits: as we're adapting minTxFee to never be higher than
...
CAmount CWallet::GetFleetCreditsPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority)
{
    // payTxFee is the user-set global for desired feerate
    return GetFleetCreditsPriorityFee(tx, nTxBytes, nPriority, payTxFee.GetFee(nTxBytes));
}
CAmount CWallet::GetFleetCreditsPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority, CAmount targetFee)
{
    CAmount nFeeNeeded = targetFee;
    // User didn't set: use -txconfirmtarget to estimate...
    if (nFeeNeeded == 0) {
        nFeeNeeded = GetFleetCreditsFeeRate(nPriority).GetFee(nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // Fleet Credits: as we're adapting minTxFee to never be higher than
    //           payTxFee unless explicitly set, this should be fine
...
DBErrors CWallet::LoadWallet(bool& fFirstRunRet)
{
    ...
    uiInterface.SafeLoadWallet(this);
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CFleetCreditsAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CFleetCreditsAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CFleetCreditsAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CFleetCreditsAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CFleetCreditsAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CFleetCreditsAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CFleetCreditsAddress(dest).ToString(), key);
}
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (GetBoolArg("-zapwallettxes", false)) {
        uiInterface.SafeInitMessage(_("Zapping all transactions from wallet..."));
    ...
    if (chainActive.Tip() && chainActive.Tip() != pindexRescan)
    {
        ...
        uiInterface.SafeInitMessage(_("Rescanning..."));
